# backend/app/ingestion/providers/ats_extractors/moka.py
...
_HOST = "app.mokahr.com"


class MokaAtsExtractor:
    platform = "moka"
    host = _HOST
    ...
    async def fetch_list(
        ...
        *,
        ...
        page = await renderer.render(url, allowed_hosts={_HOST}, wait_selector=".position-list-item, .position-card")
        challenge = self._access_challenge(page)
        if challenge is not None:
            return self._failed(url, challenge)
        result = parse_html_job_list(page.html, platform="moka")
    ...
    def _failed(url: str, error_code: str) -> tuple[RawDocument, AtsListResult]:
        document = RawDocument(
            provider="ats_moka", external_id=None, url=HttpUrl(url), title=None,
