...
class JobDeduplicator:
    ...
    async def resolve(self, company_id: UUID, candidate: JobCandidate) -> JobMatch:
        ...
        eligible = [
            ...
            if item.city == normalized.normalized_city and self._types_are_compatible(item.job_type, normalized.job_type)
        ]
        best_match = max(
            (
    ...
    @staticmethod
    def _types_are_compatible(left: JobType, right: JobType) -> bool:
        return left == right or left is JobType.UNKNOWN or right is JobType.UNKNOWN
