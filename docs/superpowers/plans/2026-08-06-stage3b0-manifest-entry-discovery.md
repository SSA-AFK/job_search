...
Fresh Task 8 evidence after gate repair on 2026-08-08 reports `729 passed, 7 skipped, 1 warning` for the first complete offline group and `221 passed` for the second. The prior filing display regression is closed: `5464a92` preserves the raw `filing_number` display value while maintaining its normalized key, and `8153f64` enforces `(filing_type, normalized_filing_number)` uniqueness with collision preflight/guard before the `0009` constraint switch. Ruff passed and mypy passed for 98 source files. The warning remains the pre-existing intentional non-integer `salary_months` Pydantic serializer warning. Default tests did not enable network, Redis, model API, browser, or job-list providers.
