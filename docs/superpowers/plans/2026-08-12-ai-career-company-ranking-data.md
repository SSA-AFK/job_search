...
- Excel 已有的工商、阶段和行业字段不得重复查询；每家公司按缺口最多执行四个业务能力调用。
...
**验收：** 能确定性调用已验证的四类能力，单家公司正常业务调用不超过四类，且供应商密钥与原始响应不落库、不进日志。
...
- [ ] 测试 100 家 × 四类的上界为 400 个逻辑调用；已有成功数据、重复运行和不适用维度都会减少实际调用。
...
**最终验收：** 固定 100 家样本可重复生成内部校准榜；单家公司不超过四类天眼业务调用；每个有效得分都有证据；敏感与供应商内部字段不落库、不进日志、不出公共 API；现有公司搜索、详情和职位流程无回归。
