...
**目标：** 对既有 AI 行业 100 家确定性样本，以每家公司最多六类天眼 AI 调用和独立公开证据形成面向 AI 求职者的长期职业价值内部校准榜。

**架构：** 将天眼 AI 视为仅内部使用的结构化供应商边界。适配器通过字段白名单把六类响应投影成最小化事件，不保存原始响应；官网、政府及审核后的行业组织来源继续提供用户可见证据。评分服务先生成五个维度的原始证据分，再按企业发展阶段校准并形成版本化快照。首批结果仅通过内部 CLI/报告访问。
...
- 每家公司最多执行六个业务能力调用；实体搜索只在无法由现有企业全称/信用代码唯一锚定时执行，并单独计入运行审计。
...
| 文件 | 职责 |
| --- | --- |
| `backend/app/rankings/tianyancha/contracts.py` | 六类调用的最小输入、白名单输出和错误契约。 |
| `backend/app/rankings/tianyancha/client.py` | 天眼 CLI 的确定性执行、认证、超时、重试及预算控制。 |
| `backend/app/rankings/tianyancha/catalog.py` | 运行时能力发现及六类语义能力映射。 |
| `backend/app/rankings/tianyancha/projectors.py` | 将供应商响应投影成阶段、技术、市场、成长、地位和风险信号。 |
| `backend/app/rankings/tianyancha/service.py` | 对试点成员执行去重、断点续跑和最多六类调用。 |
...
**验收：** 可以审计六类调用和评分输入，但无法从持久化结构恢复供应商原始响应或个人敏感信息。

## Task 3：实现天眼六类调用客户端与能力映射
...
- [ ] 将真实工具稳定映射到六个内部能力：综合画像、融资成长、知识产权、经营验证、经营风险、历史趋势。
- [ ] 客户端使用参数数组启动子进程，不拼接 shell 字符串；Windows 包装器也不得把公司名直接插入 shell 命令。
- [ ] 配置增加 CLI 路径、超时、每公司最大六类、批次并发、重试次数；API Key 只从天眼 CLI 本地配置或环境密钥读取。
...
**验收：** 能确定性调用已验证的六类能力，单家公司正常业务调用不超过六类，且供应商密钥与原始响应不落库、不进日志。
...
- [ ] 为六类响应分别写契约夹具，夹具同时包含允许字段和电话、人员、地址等禁止字段。
- [ ] 综合画像只投影经营状态、成立时间、规模分档、融资阶段和上市状态。
...
- [ ] 历史趋势只投影阶段判定需要的扩张、收缩、上市、并购和资本方向，不保存完整工商流水。
...
**验收：** 六类响应均能生成最小化、可版本化的内部信号；供应商新增未知字段默认被丢弃。
...
- [ ] 在调度前和每次调用前双重检查六类预算；批量接口减少往返但仍按返回公司数记录逻辑调用。
- [ ] 每家公司完成后立即提交最小化信号和运行状态，进程中断时已完成公司不回滚。
- [ ] CLI 进度只输出数量和稳定错误码：总数、已完成、待复核、失败、跳过；不输出公司原始响应。
- [ ] 测试 100 家 × 六类的上界为 600 个逻辑调用；已有成功数据、重复运行和部分失败都会减少实际调用。
...
- [ ] 完成天眼认证后先对 3 家跨阶段样本运行六类调用，人工检查投影字段与禁止字段。
...
- [ ] 文档记录运行命令、六类能力映射版本、评分规则版本、数据更新时间和人工复核清单；不得写入 API Key。

**最终验收：** 固定 100 家样本可重复生成内部校准榜；单家公司不超过六类天眼业务调用；每个有效得分都有证据；敏感与供应商内部字段不落库、不进日志、不出公共 API；现有公司搜索、详情和职位流程无回归。
