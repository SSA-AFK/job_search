...
Task 8 gate repair 后的 fresh 完整离线命令结果为：第一组 `729 passed / 7 skipped / 1 warning`；第二组 `221 passed`，此前 company detail API 的备案号展示回归已关闭；Ruff clean；mypy `98 source files` clean。warning 是既有 intentional 非整数 `salary_months` Pydantic serializer warning，尚需 warning assertion 或无 warning 修复。默认测试未启用网络、Redis、模型 API、浏览器或职位列表 provider。
