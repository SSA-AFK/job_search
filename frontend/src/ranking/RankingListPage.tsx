...
function RankingRow({ member, returnTo }: { member: RankingMember; returnTo: string }) {
  const detailState = { from: returnTo };
  return <li className="ranking-row">
    <div className={`rank-mark ${member.rank && member.rank <= 3 ? "rank-mark--top" : ""}`}>{member.rank ?? "—"}</div>
    <div className="ranking-company"><div><Link to={`/companies/${member.company_id}`} state={detailState}>{member.company_name}</Link><span>{stageLabels[member.company_stage]}</span>{member.active_job_count ? <span className="opportunity-tag">在招 {member.active_job_count}</span> : null}</div><p>{member.reason}</p></div>
    <ScoreBars member={member} />
    <div className="ranking-total"><strong>{member.total_score}</strong><span>总分</span></div>
    <Link className="ranking-open" to={`/companies/${member.company_id}`} state={detailState} aria-label={`查看${member.company_name}详情`}><ArrowRight size={18} /></Link>
  </li>;
}
...
export function RankingListPage() {
  ...
  const stage = stageParam === "early" || stageParam === "growth" || stageParam === "mature" ? stageParam : undefined;
  const returnTo = stage ? `/list?stage=${stage}` : "/list";
  ...
  return <main><Header /><div className="content-width ranking-workspace">
    ...
    <div className="ranking-toolbar"><div><ListFilter size={17} /><span>发展阶段</span>{([undefined, "early", "growth", "mature"] as const).map(value => <button className={stage === value ? "active" : ""} key={value ?? "all"} type="button" onClick={() => setSearchParams(value ? { stage: value } : {})}>{value ? stageLabels[value] : "全部"}</button>)}</div>
    ...
    {ranked ? <section aria-labelledby="ranked-heading"><div className="ranking-section-heading"><h2 id="ranked-heading">正式榜单</h2><span>{ranked.total} 家</span></div><ol className="ranking-list">{ranked.items.map(member => <RankingRow key={member.company_id} member={member} returnTo={returnTo} />)}</ol></section> : null}
    {observation && observation.items.length ? <section className="observation-section" aria-labelledby="observation-heading"><div className="ranking-section-heading"><div><h2 id="observation-heading">观察池</h2><p>AI 相关性证据尚不足，不分配正式名次。</p></div><span>{observation.total} 家</span></div><ul className="ranking-list">{observation.items.map(member => <RankingRow key={member.company_id} member={member} returnTo={returnTo} />)}</ul></section> : null}
